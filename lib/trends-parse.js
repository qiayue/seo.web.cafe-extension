...
(function (root) {
  ...
  // 对比：谷歌趋势一次最多比 5 个词，网址里 q=词1,词2,…（逗号隔开）。热度 0~100 是「这一次查询」里归一化的——
  // 这次查的所有词、所有时间点里最高的那个记 100，其余按比例折算；所以分开查的两个词各自的 80 不代表一样热，
  // 要比高低只能放进同一次查询。插件里「词」统一写成逗号隔开的一串（小写、去空格、去重）
  var MAX_COMPARE = 5;
  function normKeywords(s) {
    var seen = {}, out = [];
    String(s || "").split(/[,，]/).forEach(function (w) {
      w = w.replace(/\s+/g, " ").trim().toLowerCase();
      if (w && !seen[w] && out.length < MAX_COMPARE) { seen[w] = 1; out.push(w); }
    });
    return out.join(",");
  }

  /** 从接口网址的 req 参数里读出查的是哪个词、什么类型（QUERY = 相关查询，ENTITY = 相关主题）。
   *  曲线接口里对比的每个词各占一个 comparisonItem：拼成「词1,词2」；相关查询接口每个词单独一个，就是那一个词 */
  function parseReq(url) {
    ...
    var first = function (r) { return r && r.keyword && r.keyword[0] && typeof r.keyword[0].value === "string" ? r.keyword[0].value : ""; };
    var words = [];
    var items = (req && req.comparisonItem) || [];
    for (var i = 0; i < items.length; i++) words.push(first(items[i] && items[i].complexKeywordsRestriction));
    if (!items.length) words.push(first(req && req.restriction && req.restriction.complexKeywordsRestriction));
    return { keyword: normKeywords(words.join(",")), keywordType: String((req && req.keywordType) || "") };
  }

  /** 热度曲线 → [{t, v, vs?, p?}]。v 是第一个词；对比几个词时 vs = 每个词在这个点上的热度（同一把尺子，可以直接比）。
  ...
  function parseTimeline(text) {
    ...
    for (var i = 0; i < rows.length && points.length < MAX_POINTS; i++) {
      ...
      if (r.value.length > 1) {
        pt.vs = [];
        for (var k = 0; k < r.value.length && k < MAX_COMPARE; k++) { var x = Number(r.value[k]); pt.vs.push(isFinite(x) ? Math.max(0, Math.min(100, Math.round(x))) : 0); }
      }
  ...
  function buildTrendsUrl(opts) {
    var raw = String((opts && opts.keyword) || "");
    var words = /[,，]/.test(raw) ? normKeywords(raw).split(",").filter(Boolean) : [raw.replace(/\s+/g, " ").trim()];
    if (!words[0] || words.some(function (w) { return w.length > 100; })) throw new Error("关键词为空或太长");
    ...
    return "https://trends.google.com/trends/explore?date=" + encodeURIComponent(date)
      + (geo ? "&geo=" + geo : "") + "&q=" + words.map(encodeURIComponent).join(",");
  }

  var api = { normKeywords: normKeywords, MAX_COMPARE: MAX_COMPARE, parseBody: parseBody, parseReq: parseReq, parseTimeline: parseTimeline, parseRelated: parseRelated, buildTrendsUrl: buildTrendsUrl,
