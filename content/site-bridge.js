...
//   插件 → 页面：hello（我在，版本 x）/ trends:result（取数结果）
//   页面 → 插件：ping（在吗）/ trends:fetch（请去谷歌趋势取这个词）
  ...
  window.addEventListener("message", function (e) {
    ...
    try {
      chrome.runtime.sendMessage({ type: "trends:fetch", requestId: d.requestId, keyword: d.keyword, geo: d.geo || "", date: d.date }, function (res) {
        ...
        if (!res || !res.ok) fail((res && res.error) || "插件没接住请求");
        // 接住了：结果稍后由后台经 chrome.tabs.sendMessage 送回来（见下面）
      });
    } catch (err) {
