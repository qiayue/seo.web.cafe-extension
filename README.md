...
3. **在侧边栏里直接查谷歌趋势。** 输入关键词、选时间范围和地区，点「查询」：插件走同一条取数路（后台开谷歌趋势标签页、抄回数据），曲线、最高点、第一次有热度的时间、相关查询和原始 JSON 直接显示在侧边栏里。不经过 Agent、不扣积分、不进网站缓存——用来单独核对插件取到的数对不对。勾上「取完不关谷歌趋势标签页」可以拿网页对照着看。
